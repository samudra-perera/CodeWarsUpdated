...
function firstNonRepeatingLetter(s) {
    if(s.length == 0) return '' // an empty string would return an empty string
    let obj = {}
    //First for loop to create a set of and count the number of isntance of the letters
    for(let i = 0; i  < s.length; i++) {
    ...
    for(let i = 0; i < s.length; i++) { //For loop to check the first character that has a value of 1 in the object
        let char1 = s.toLowerCase().charAt(i)
        if(obj[char1] == 1) {
            return s.charAt(i)
        }
    }

    return '' //If any character repeats more than once return an empty string
  }
